...
public class GlobalConfigurationMocker {
    ...
    public void mockLoadNewestComponent(Component.State state) throws Exception {
        for (String node : IlesComponentFactory.getInstance().getRegisteredClassNames()) {
            ...
            if(component.getType().equals("Int")) {
                component.setVersion(Version.createReleaseVersion("1.0"));
            } else {
                component.setVersion(Version.createReleaseVersion("R1A01"));
            }
     ...
     *
     * @param components the VISE channels to return
     *********************************************************************/
    public void mockLoadAllComponents(Component... components) throws IOException {
