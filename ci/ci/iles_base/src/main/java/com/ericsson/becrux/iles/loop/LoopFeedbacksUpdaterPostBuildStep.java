...
import static com.ericsson.becrux.iles.eventhandler.strategies.ITREventStrategy.ITR_REQUESTER;
...
public class LoopFeedbacksUpdaterPostBuildStep extends NwftPostBuildStep {
    ...
    public boolean perform(final AbstractBuild<?, ?> build, final Launcher launcher, final BuildListener listener)
        ...
        btf.setRequester(getITRJobRequester(build));
     ...
     * @return
     */
    private String getITRJobRequester(final AbstractBuild<?, ?> build) {
        ITREvent eITR = getFirstITREventFromBuild(build);
        if (eITR != null) {
            if (eITR.getParameters().containsKey(ITR_REQUESTER)) {
                return eITR.getParameters().get(ITR_REQUESTER);
            } else {
                throw new IllegalArgumentException("No ITR Job Requester is provided");
            }
        }
        return null;
    }
