...
public class PhoenixSchedulerBuildStep extends NwftBuildStep {
   ...
   @Override
    ...
    /**
     * Start phoenix installation for IMS node by scheduling the downstream job responsible
     * for phoenix installation.
     * @param build
     * @param launcher
     * @param listener
    ...
    private ResultParameterValue startPhoenix (AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) {
        ...
        ResultParameterValue result = new ResultParameterValue(PROCESS_NAME, PhaseStatus.SUCCESS, details, null, null);
        ...
        try {
            ...
            //get the source folder of script
            String source = ExecCommandHelper.readSymbolicLink(ilesProcessingHelper.getPhoenixScriptPath(build));
            listener.getLogger().println("The Phoenix source folder " + source);

            //add to NWFTParameter
            result.setSourcePath(source);
            ...
            if (components.size() == 0) {
                ...
                for (ComponentParameterValue param : components) {
                    ...
                    listener.getLogger().println("* " + n.getType() + " " + n.getVersion());
                }
            }
            ...
            for (ComponentParameterValue nodeParam : components) {
                ...
                if (node.isInstallable()) {
                    String description = node.getType() + " " + node.getVersion();

                    action.addParam(nodeParam);
                    action.addParam(channel);

                    // Add the Leo parameters to NwftParameterValue
                    ...
                    /*
                        Right now common params will include:
                            + ITREventStrategy.QUICK_TEST
                            + ITREventStrategy.CUSTOM_PROVISIONING_TOOL_PATH
                            + ITREventStrategy.CUSTOM_TESTEXEC_TOOL_PATH
                            + ITREventStrategy.CUSTOM_PHOENIX_TOOL_PATH
                            + ITREventStrategy.INSTALLATION_ABLE
                            + ITREventStrategy.PROVISIONING_ABLE
                            + ITREventStrategy.TESTEXEC_ABLE
                     */
                    action.addParam(commonParams);
                    actions.add(action);
