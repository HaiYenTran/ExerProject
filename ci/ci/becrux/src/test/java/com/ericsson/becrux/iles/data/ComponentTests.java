...
public class ComponentTests {
	...
	public void objectSerializationTest() {
		Component before = new Mtas("1.0", Component.State.BASELINE_APPROVED);
	...
	public void listSerializationTest() {
		List<Component> before = Arrays.asList( 
				new Cscf("1.1", Component.State.NEW_BUILD),
				new Int("1.1", Component.State.BASELINE_APPROVED));
		assertNotNull(before);
	...
	public void testCreationFromITREvent() {
		...
		Component nodeFromScratch = new Cscf("1.0.1");
	...
	public void testCompareBetweenDifferentComponent() throws Exception {
		Cscf cscf = (Cscf) IlesComponentFactory.getInstance().create(Cscf.class.getSimpleName(), "1.0");
		Ibcf ibcf = (Ibcf) IlesComponentFactory.getInstance().create(Ibcf.class.getSimpleName(), "2.0");
