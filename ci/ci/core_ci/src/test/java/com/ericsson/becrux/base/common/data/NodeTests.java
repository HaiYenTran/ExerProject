...
public class NodeTests {
    class DummyComponent extends Component {
        ...
        public DummyComponent(@Nonnull String version) {
            super(version);
        }
    }
	...
	public void getDeployParametersTest() throws IllegalArgumentException, MalformedURLException, IOException {
		Component node = new BaseComponentImpl("1.0.1");
		File file = null;
		try {
    ...
    public void testEqualsNegativeTest() {
	    Component aNode = new BaseComponentImpl("1.0.1");
        assertFalse(aNode.equals(null));

        assertFalse(aNode.equals(new DummyComponent()));

        Component otherNode = new BaseComponentImpl("1.0.2", "otherArtifact");
        ...
        aNode.setArtifact("anArtifact");
        aNode.setState(Component.State.BASELINE_APPROVED);
        otherNode = new BaseComponentImpl("1.0.2", "anArtifact");
        otherNode.setState(Component.State.BASELINE_APPROVED);
    ...
    public void testEqualsPositiveTest() {
        Component aNode = new BaseComponentImpl("1.0.1", "anArtifact");
        aNode.setState(Component.State.BASELINE_APPROVED);
        assertTrue(aNode.equals(aNode));

        Component otherNode = new BaseComponentImpl("1.0.1", "anArtifact");
        otherNode.setState(Component.State.BASELINE_APPROVED);
    ...
    public void testCompareTo() {
        ...
        Component otherNode = new BaseComponentImpl("1.0.2");
        assertEquals(-1, aNode.compareTo(otherNode));

        otherNode = new BaseComponentImpl("1.0.1");
        assertEquals(0, aNode.compareTo(otherNode));

        otherNode = new DummyComponent("1.0.1");
        assertEquals(0, aNode.compareTo(otherNode));
    }
    ...
    public void testSettersGetters() {
        Component component = new BaseComponentImpl("1.0.0", Component.State.NEW_BUILD);
