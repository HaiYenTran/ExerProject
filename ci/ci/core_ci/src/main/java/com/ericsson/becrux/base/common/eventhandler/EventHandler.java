...
public class EventHandler {
    ...
    protected void handleEventBatch(List<Event> batch) throws InterruptedException {
        ...
        for (Event event : batch) {
            ...
            try {
                ...
                log.println("\n***Finished handling " + event.getType() + " ID: " + event.getID());
                log.println("|================|");
                log.println("| Status: " + result.getSimpleResult());
                log.println("|================|");

                if (!result.isSuccessful()) {
                    log.println("=> Reason: " + result.getDescription() + "\n");
    ...
    public synchronized EventHandlingResult handleEvent(Event event) throws EventHandlingException, InterruptedException {
        ...
        log.println("\n***Start handling " + event.getType() + " ID: " + event.getID());
        try {
        ...
        try {
            ...
            log.println("-> Event Validation: FAILED With Exception: " + e.getMessage());
        ...
        do {
            if (retry == 0)
                log.println("-> Calling event handling: ");
            else
                log.println("-> Handling event (retry " + retry + ")");
...
// GETTERS & SETTERS
    ...
    public List<Event> getInputEvents() {
        return Collections.unmodifiableList(inputEvents);
    }
